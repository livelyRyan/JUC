...
对比ConcurrentLinkedQueue，用数组实现的�?�度更快

> 假如长度�?8，当添加到第12个元素的时�?�在哪个序号上呢？用12%8决定
>
> 当Buffer被填满的时�?�到底是覆盖还是等待，由Producer决定
>
...
2. 定义Event工厂，用于填充队�?
   ...
   > 这里牵扯到效率问题：disruptor初始化的时�?�，会调用Event工厂，对ringBuffer进行内存的提前分�?
   >
   > GC产频率会降低

3. 定义EventHandler（消费�?�），处理容器中的元�?
...
> ProducerType有两种模�? Producer.MULTI和Producer.SINGLE
>
> 默认是MULTI，表示在多线程模式下产生sequence
>
> 如果确认是单线程生产者，那么可以指定SINGLE，效率会提升
>
> 如果是多个生产�?�（多线程），但模式指定为SINGLE，会出什么问题呢�?

## 等待策略

1�?(常用）BlockingWaitStrategy：�?�过线程阻塞的方式，等待生产者唤醒，被唤醒后，再循环�?查依赖的sequence是否已经消费�?

2，BusySpinWaitStrategy：线程一直自旋等待，可能比较耗cpu

3，LiteBlockingWaitStrategy：线程阻塞等待生产�?�唤醒，与BlockingWaitStrategy相比，区别在signalNeeded.getAndSet,如果两个线程同时访问�?个访问waitfor,�?个访问signalAll时，可以减少lock加锁次数.

4，LiteTimeoutBlockingWaitStrategy：与LiteBlockingWaitStrategy相比，设置了阻塞时间，超过时间后抛异常�??

5，PhasedBackoffWaitStrategy：根据时间参数和传入的等待策略来决定使用哪种等待策略

6，TimeoutBlockingWaitStrategy：相对于BlockingWaitStrategy来说，设置了等待时间，超过后抛异�?

7，（常用）YieldingWaitStrategy：尝�?100次，然后Thread.yield()让出cpu
